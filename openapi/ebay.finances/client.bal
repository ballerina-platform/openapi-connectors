...
@display {label: "eBay Finances", iconPath: "icon.png"}
public isolated client class Client {
    ...
    #
    # + payoutId - The unique identfier of the payout is passed in as a path parameter at the end of the call URI. The getPayouts method can be used to retrieve the unique identifier of a payout, or the user can check Seller Hub to get the payout ID. 
    # + return - Success 
    remote isolated function getPayout(string payoutId) returns Payout|error {
        string resourcePath = string `/payout/${getEncodedUri(payoutId)}`;
        Payout response = check self.clientEp->get(resourcePath);
        return response;
    }
    ...
    remote isolated function getPayoutSummary(string? filter = ()) returns PayoutSummaryResponse|error {
        string resourcePath = string `/payout_summary`;
        map<anydata> queryParam = {"filter": filter};
        resourcePath = resourcePath + check getPathForQueryParam(queryParam);
        PayoutSummaryResponse response = check self.clientEp->get(resourcePath);
        return response;
    }
    ...
    remote isolated function getTransactions(string? filter = (), string? sort = (), string? 'limit = (), string? offset = ()) returns Transactions|error? {
        string resourcePath = string `/transaction`;
        map<anydata> queryParam = {"filter": filter, "sort": sort, "limit": 'limit, "offset": offset};
        resourcePath = resourcePath + check getPathForQueryParam(queryParam);
        Transactions? response = check self.clientEp->get(resourcePath);
        return response;
    }
    ...
    remote isolated function getTransfer(string transferId) returns Transfer|error {
        string resourcePath = string `/transfer/${getEncodedUri(transferId)}`;
        Transfer response = check self.clientEp->get(resourcePath);
        return response;
    }
    ...
    remote isolated function getSellerFundsSummary() returns SellerFundsSummaryResponse|error? {
        string resourcePath = string `/seller_funds_summary`;
        SellerFundsSummaryResponse? response = check self.clientEp->get(resourcePath);
        return response;
    }
}
