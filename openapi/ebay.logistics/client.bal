...
public isolated client class Client {
    ...
    public isolated function init(ConnectionConfig config, string serviceUrl = "https://api.ebay.com/sell/logistics/v1_beta") returns error? {
    ...
    remote isolated function createShippingQuote(ShippingQuoteRequest payload) returns ShippingQuote|error {
        string resourcePath = string `/shipping_quote`;
        http:Request request = new;
        json jsonBody = check payload.cloneWithType(json);
        request.setPayload(jsonBody, "application/json");
        ShippingQuote response = check self.clientEp->post(resourcePath, request);
        return response;
    }
    ...
    remote isolated function createFromShippingQuote(CreateShipmentFromQuoteRequest payload) returns Shipment|error {
        string resourcePath = string `/shipment/create_from_shipping_quote`;
        http:Request request = new;
        json jsonBody = check payload.cloneWithType(json);
        request.setPayload(jsonBody, "application/json");
