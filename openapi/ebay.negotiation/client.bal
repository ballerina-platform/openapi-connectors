...
# The Negotiations API gives sellers the ability to proactively send discount offers to buyers who have shown an "interest"
...
# original listing price.  For details about how seller offers work, 
...
public isolated client class Client {
    ...
    # The HTTP client initialization requires setting the API credentials. Create a [developer account](https://developer.ebay.com/api-docs/static/creating-edp-account.html)  and obtain tokens following [this guide](https://developer.ebay.com/api-docs/static/oauth-tokens.html).
    #
    ...
    # This method evaluates a seller's current listings and returns the set of IDs that are eligible for a seller-
    # initiated discount offer to a buyer. A listing ID is returned only when one or more buyers have shown an 
    # &quot;interest&quot; in the listing. If any buyers have shown interest in a listing, the seller can initiate a 
    # &quot;negotiation&quot; 
    ...
    #
    # + xEbayCMarketplaceId - The eBay marketplace on which you want to search for eligible listings. For a complete l
    # ist of supported marketplaces, see Negotiation API requirements and restrictions.
    ...
    # response contains the first 10 results from the complete list of items retrieved by the call. If offset is 10 and 
    # limit is 20, the first page of the response contains items 11-30 from the complete result set. Default: 0
    # + return - Success
    ...
    # interest in a listing, they become &quot;eligible&quot; to receive a seller-initiated offer to purchase the item(s). 
    # Sellers use findEligibleItems to get the set of listings that have interested buyers. If a listing has interested 
    # buyers, sellers can use this method (sendOfferToInterestedBuyers) to send an offer to the buyers who are 
    # interested in the listing. The offer gives buyers the ability to purchase the associated listings at a discounted 
    # price. For details about how to create seller offers to buyers, see Sending offers to buyers.
    #
    # + xEbayCMarketplaceId - The eBay marketplace on which your listings with &quot;eligible&quot; buyers appear. For a 
    # complete list of supported marketplaces, see Negotiation API requirements and restrictions.
    ...
    remote isolated function sendOfferToInterestedBuyers(string xEbayCMarketplaceId, CreateOffersRequest payload) 
        ...
        SendOfferToInterestedBuyersCollectionResponse response = check self.clientEp->post(path, request, 
        headers = accHeaders, targetType=SendOfferToInterestedBuyersCollectionResponse);
        return response;
    }
}
