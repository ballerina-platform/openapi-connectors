...
public client class Client {
    ...
    public isolated function init(ClientConfig clientConfig, string serviceUrl = "https://api.notion.com") 
    returns error? {
    ...
    remote isolated function appendBlockChildren(string id, PageUpdateRequestBody payload, string notionVersion = "2021-05-13") returns 
    ChildBlockContent|error {
        string path = string `/v1/blocks/${id}/children`;
        map<any> headerValues = {"Notion-Version": notionVersion};
        map<string|string[]> accHeaders = getMapForHeaders(headerValues);
        ...
        ChildBlockContent response = check self.clientEp->patch(path, request, headers = accHeaders, targetType = 
        ChildBlockContent);
        return response;
    }
    # Retrieve a database
    ...
    remote isolated function queryDatabase(string id, DatabaseContent payload, string notionVersion = "2021-05-13") returns 
    DatabaseResponse|error {
        ...
        json jsonBody = check payload.cloneWithType(json);
        request.setPayload(jsonBody);
        DatabaseResponse response = check self.clientEp->post(path, request, headers = accHeaders, targetType = 
        DatabaseResponse);
        return response;
    }
    # Retrieve a Page
    ...
    remote isolated function updatePageProperties(string id, PageContent payload, string notionVersion = "2021-05-13") returns 
    PageUpdatedProperties|error {
        string path = string `/v1/pages/${id}`;
        map<any> headerValues = {"Notion-Version": notionVersion};
        map<string|string[]> accHeaders = getMapForHeaders(headerValues);
        http:Request request = new;
        json jsonBody = check payload.cloneWithType(json);
        request.setPayload(jsonBody);
        PageUpdatedProperties response = check self.clientEp->patch(path, request, headers = accHeaders, targetType = 
