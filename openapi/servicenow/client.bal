...
# + secureSocketConfig - Secure Socket connection configuration 
...
public client class Client {
    ...
    # + serviceUrl - Connector server URL(Your ServiceNow instance URL)
    ...
    remote isolated function getRecordById(string tableName, string sys_id, string? sysparmDisplayValue = (), boolean? sysparmExcludeReferenceLink = (), string? sysparmFields = (), boolean? sysparmQueryNoDomain = (), string? sysparmView = (), string? accept = ()) returns json|error {
        string  path = string `/api/now/table/${tableName}/${sys_id}`;
    ...
    remote isolated function updateRecord(string tableName, string sys_id, json payload, string? sysparmDisplayValue = (), boolean? sysparmExcludeReferenceLink = (), string? sysparmFields = (), boolean? sysparmInputDisplayValue = (), boolean? sysparmQueryNoDomain = (), string? sysparmView = (), string? accept = (), string? contentType = (), string? xNoResponseBody = ()) returns json|error {
        string  path = string `/api/now/table/${tableName}/${sys_id}`;
    ...
    remote isolated function deleteRecord(string tableName, string sys_id, boolean? sysparmQueryNoDomain = (), string? accept = ()) returns http:Response|error {
        string  path = string `/api/now/table/${tableName}/${sys_id}`;
    ...
    # + xNoResponseBody - By default, responses include body content detailing the new record. Set this header to true in the request to suppress the response body.
    # + return - If successful, returns a JSON object that includes updated record of the table. Otherwise returns the relevant error.
    remote isolated function patchRecordById(string tableName, string sys_id, json payload, string? sysparmDisplayValue = (), string? sysparmFields = (), boolean? sysparmInputDisplayValue = (), boolean? sysparmQueryNoDomain = (), string? sysparmView = (), string? accept = (), string? contentType = (), string? xNoResponseBody = ()) returns json|error {
        string  path = string `/api/now/table/${tableName}/${sys_id}`;
    ...
    remote isolated function getImportSet(string stagingTableName, string sys_id, string? accept = ()) returns json|error {
        string  path = string `/api/now/import/${stagingTableName}/${sys_id}`;
        map<any> headerValues = {"Accept": accept};
        map<string|string[]> accHeaders = getMapForHeaders(headerValues);
        json response = check self.clientEp-> get(path, accHeaders, targetType = json);
        return response;
    ...
    # + accept - Data format of the response body.
    # + return - If successful, returns a JSON object. Otherwise returns the relevant error.
    remote isolated function getValuesByFieldNameInCase(string field_name, string? sysparmDependentValue = (), decimal? sysparmLimit = (), decimal? sysparmOffset = (), string? sysparmReferenceFieldColumns = (), string? sysparmQuery = (), string? sysparmRefQualInput = (), string? accept = ()) returns json|error {
        string  path = string `/api/sn_customerservice/case/field_values/${field_name}`;
    ...
    # + accept - Data format of the response body.
    # + return - If successful, returns a JSON object. Otherwise returns the relevant error.
    remote isolated function getValuesByCaseIdAndFieldName(string field_name, string id, string? sysparmDependentValue = (), decimal? sysparmLimit = (), decimal? sysparmOffset = (), string? sysparmReferenceFieldColumns = (), string? sysparmQuery = (), string? sysparmRefQualInput = (), string? accept = ()) returns json|error {
        string  path = string `/api/sn_customerservice/case/${id}/field_values/${field_name}`;
        map<anydata> queryParam = {"sysparm_dependent_value": sysparmDependentValue, "sysparm_limit": sysparmLimit, "sysparm_offset": sysparmOffset, "sysparm_reference_field_columns": sysparmReferenceFieldColumns, "sysparm_query": sysparmQuery, "sysparm_ref_qual_input": sysparmRefQualInput};
        path = path + check getPathForQueryParam(queryParam);
        map<any> headerValues = {"Accept": accept};
        map<string|string[]> accHeaders = getMapForHeaders(headerValues);
...
# + return - Generated Path or error at failure of client initialization
...
# + return - Generated map or error at failure of client initialization
